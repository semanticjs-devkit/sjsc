...
class Initialize extends sjsc_command_1.default {
    async run() {
        ...
        await fs_extra_1.ensureFile(`src/index.ts`);
    ...
    async configurePackageJson(args) {
        ...
        pkgJson.scripts = Object.assign(Object.assign({}, pkgJson.scripts), { "build": "concurrently \"npm run build:es\" \"npm run build:cjs\"", "build:es": "tsc -p tsconfig.es.json", "build:cjs": "tsc -p tsconfig.cjs.json", "deploy": "npm version patch && npm run deploy:all", "deploy:all": "rimraf packages && npm run build && npm publish ./packages --access public", "test": "jest", "test:coverage": "jest --coverage", "watch": "npm run watch:es", "watch:es": "tsc -p tsconfig.es.json -w", "watch:cjs": "tsc -p tsconfig.cjs.json -w" });
...
module.exports = {
  ...
  transform: {
    ...
    async inintializeSJSConfig(args) {
        const sjsCfg = {
    ...
    async inintializeTsConfig() {
        ...
        await fs_extra_1.writeFile("tsconfig.json", JSON.stringify(tsconfig));
    }
    async inintializeTsConfigCJS() {
        ...
        await fs_extra_1.writeFile("tsconfig.cjs.json", JSON.stringify(tsconfig));
    }
    async inintializeTsConfigES() {
        ...
        await fs_extra_1.writeFile("tsconfig.es.json", JSON.stringify(tsconfig));
    }
}
