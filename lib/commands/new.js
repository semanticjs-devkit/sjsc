...
class New extends sjsc_command_1.default {
    async run() {
...
export default class ${name} {
  ...
  protected toggleExample() {
    ...
    async addClassToTemplate(name, path) {
        const sjsCfg = await this.readSJSConfig();
        const moduleString = await fs_extra_1.readFile(`src/${sjsCfg.Repository}.ts`);
        // const moduleLines = await StringUtils.
    }
}
exports.default = New;
New.aliases = ["n"];
